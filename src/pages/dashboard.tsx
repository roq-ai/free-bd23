...
import { FreelancerProfileListPage as ShowCaseList } from 'pages/freelancer-profiles';
...
function DashboardPage() {
  ...
  const freelancerProfileStat = useSWR<PaginatedInterface<FreelancerProfileInterface>>(
    () => `/freelancer-profiles`,
    () => getFreelancerProfiles({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: freelancerProfileStat?.data?.totalCount,
      isLoading: freelancerProfileStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Freelancer Profiles
                </Text>
                <NextLink href={`freelancer-profiles`} passHref legacyBehavior>
                  <Button
